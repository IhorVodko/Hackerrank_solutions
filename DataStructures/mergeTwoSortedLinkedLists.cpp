...
SinglyLinkedListNode * mergeLists(
        SinglyLinkedListNode * _head1
    ,   SinglyLinkedListNode * _head2
){
    auto node = new SinglyLinkedListNode(0);
    auto beforeHead = node;
    while(_head1 && _head2){
        if(_head1->data <= _head2->data){
            node->next = _head1;
            _head1 = _head1->next;
        }else{
            node->next = _head2;
    ...
    node->next = _head1 ? _head1 : _head2;
